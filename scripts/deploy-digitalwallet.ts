...
import { DigitalWallet } from "../typechain-types";

async function main() {
  console.log("🚀 Starting DigitalWallet deployment to Monad Testnet...\n");
  ...
  // Deploy DigitalWallet
  console.log("📦 Deploying DigitalWallet contract...");
  const DigitalWalletFactory = await ethers.getContractFactory("DigitalWallet");
  ...
  const digitalWallet: DigitalWallet = await DigitalWalletFactory.deploy({
    gasLimit: gasLimit
  });
  ...
  console.log("✅ Digital Wallet deployed successfully!");
